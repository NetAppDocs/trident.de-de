...
NetApp Trident unterstützt das Klonen von Volumes bei Verwendung der  `ontap-nas` ,  `ontap-san` ,  `solidfire-san` , Und  `gcp-cvs` Speichertreiber. Wenn Sie die oder `ontap-nas-economy`-Treiber verwenden `ontap-nas-flexgroup`, wird das Klonen nicht unterstützt. Wenn Sie ein neues Volume von einem vorhandenen Volume erstellen, wird ein neuer Snapshot erstellt.
