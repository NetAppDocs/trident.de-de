...
* Aktualisieren Sie die Storage-Klasse, um den `fsType` Parameter (z. B. `fsType: "ext4"` ) in der OpenShift-Virtualisierungsumgebung zu erhalten. Setzen Sie bei Bedarf den Volume-Modus explizit auf Blockieren, indem Sie den Parameter im `dataVolumeTemplates` verwenden `volumeMode=Block`, um CDI zum Erstellen von Block-Daten-Volumes zu benachrichtigen.
