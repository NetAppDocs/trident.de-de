...
* *Kubernetes*:
+
** Ein Wettlaufproblem zwischen CSI-Veröffentlichung und Volume-Verschiebung wurde behoben, das dazu führen konnte, dass ein Volume mit veralteten Zugriffsinformationen angehängt wurde.
** Es wurde ein Wettlaufproblem behoben, bei dem NodeGetInfo leere Topologiebezeichnungen zurückgegeben werden konnten, bevor die Knotenregistrierung abgeschlossen war, was zu Fehlern bei der topologiebasierten Bereitstellung führte.
** Ein seltener NVMe-Verbindungsabbruchfehler wurde behoben, der dazu führen konnte, dass ein gemeinsam genutztes Subsystem entfernt wurde, während ein anderer Pod noch verbunden wurde.
** Fehler beim Veröffentlichen von NVMe PVC-Klonen (und eine fehlerhafte Zuordnung des übergeordneten Namespace) beim Erstellen von Klonen eines gemounteten Volume unter gleichzeitiger Ausführung wurden behoben.
** Fehler bei der Erstellung von Snapshots für PVCs auf ONTAP SAN Economy Backends mit einem nameTemplate wurden behoben.
...
* *Kubernetes*:
+
** Optionale Option  `storageAPITimeout` (Sekunden) in der ONTAP Backend-Konfiguration hinzugefügt, um die REST- und ZAPI-API-Client-Timeouts pro Backend festzulegen.
...
* *Kubernetes*:
+
** Allgemeine Verfügbarkeit (GA) für Parallelverarbeitung bei den ONTAP-NAS-Economy- und ONTAP-SAN-Economy-Treibern wurde hinzugefügt, wodurch diese Funktionalität aus der Tech Preview übernommen wurde. Zudem wurde die GA-Unterstützung für Parallelverarbeitung bei den Azure NetApp Files (ANF) und SolidFire Treibern hinzugefügt.
** Unterstützung für Kubernetes 1.36 hinzugefügt.
** Allgemeine Verfügbarkeit (GA) für die automatische Volumenerweiterung mit den Google Cloud NetApp Volumes (GCNV) NAS- und SAN-Treibern.
** Unified ONTAP Balanced Placement für optimale Aggregatauswahl bei der Bereitstellung neuer Volumes hinzugefügt (link:https://github.com/NetApp/trident/issues/64["Problem #64"]).
** ONTAP AFX-Unterstützung für den ONTAP NAS Flexgroup-Treiber hinzugefügt.
** Integration mit dem NetApp Shift Toolkit wurde hinzugefügt, um die Migration virtueller Maschinen bei Verwendung des Red Hat OpenShift Migration Toolkit for Virtualization zu vereinfachen.
** Cloud Identity-Unterstützung für den GCNV SAN-Treiber hinzugefügt.
** Unterstützung für untergeordnete Volumes bei der Trident Volume Publication (TVP) Propagation und den Autogrow-Workflows hinzugefügt.
** Unterstützung für Volume-Updates und Volume-Gruppen-Snapshots bei aktiviertem Parallelbetrieb von Trident.
** Dynamische gRPC-Ratenbegrenzung für GCNV wurde hinzugefügt.
** Eine Controller-Bereitschaftsprüfung wurde hinzugefügt und die Standardeinstellungen für Kubernetes API QPS und Burst für den Controller und die Sidecars angepasst.
** Verbesserte Abfolge der Knotenoperationen durch Entkopplung der Schritte Anfügen, Verschlüsselungseinrichtung und Dateisystemformatierung.
** Die Root-Dateisysteme des Trident-Containers wurden zur Verbesserung der Sicherheit als schreibgeschützt eingebunden.
** Die Entfernung von Snapshots von Klonvolumes in ONTAP Backends nach dem Best-Effort-Prinzip wurde hinzugefügt, wenn der Quell-Snapshot eines Klonvolumes gelöscht wird.
** Die Erkennung von Backup-Klon-PVCs wurde hinzugefügt, um zu verhindern, dass temporäre PVCs, die von Veeam Kasten, Trilio und Cohesity erstellt wurden, in der ONTAP Volume Recovery Queue hängen bleiben.
** Erweiterte AWS-ARN-Verarbeitung zur Unterstützung spezieller Regionen.
** Verhindern, dass Flexvols während der Größenänderungsoperation für den ONTAP SAN ECONOMY Treiber verkleinert werden.
...
* *[Tech Preview]*: Unterstützung für den Import von ONTAP-NAS-Economy qtree hinzugefügt.
* *[Tech Preview]*: Unterstützung für Parallelverarbeitung für die ASA-R2 (SAN und NVMe) Treiber wurde hinzugefügt.
* *[Tech Preview]*: GCNV ONTAP-Modus HTTP-Transport für die ONTAP-SAN und ONTAP-NAS Treiber hinzugefügt.
...
* *Kubernetes*:
+
** Race Conditions und die Zustandsverwaltung bei gleichzeitigen Publish/Unpublish-, Klon-, Cache- und Backend-Update-Operationen wurden behoben.
** Race Conditions bei Exportrichtlinien und gleichzeitige Behandlung von Publish/Unpublish-Vorgängen für untergeordnete Volumes und schreibgeschützte Klone in den ONTAP-NAS und ONTAP-NAS-Economy Treibern wurden behoben.
** Die Erkennung von ONTAP-NAS-Economy FlexVol nach einem MetroCluster Failover wurde behoben, wenn sich die Snapshot-Richtlinien unterscheiden (link:https://github.com/NetApp/trident/issues/1082["Problem #1082"]).
** Behobene Wiederholungsversuche beim Löschen von ONTAP NAS Economy Volumes, die langlaufende Qtree-Löschvorgänge unterbrochen hatten (link:https://github.com/NetApp/trident/issues/1121["Problem #1121"]).
** Das Import- und Größenänderungsverhalten von ONTAP-SAN und ONTAP-SAN-Economy wurde korrigiert, einschließlich fsType-Validierung, Verarbeitung von Volume-Metadaten und Verhalten im Autogrow-Modus.
** Ein Race Condition im NVMe/TCP-Namespace bei gleichzeitiger Volume-Erstellung wurde behoben (link:https://github.com/NetApp/trident/issues/1089["Problem #1089"]).
** Fehler bei der iSCSI-Größenänderung, die auftraten, wenn verfügbare Pfade und Portale nicht übereinstimmten, wurden behoben und das Verhalten bei wiederholten Größenänderungen verbessert.
** Fehler beim Löschen von ONTAP SAN Volumes, die durch belegte Volumes verursacht wurden, wurden behoben.
** Die Fehlerbehandlung für ONTAP REST wurde behoben und das Verhalten bei SVM-Zeitüberschreitungen verbessert.
** Behobene Probleme bei der Handhabung von GCNV SAN Zonenpools und Hostgruppen, normalisierte Konvertierung von Volumenkennungen und normalisierte Erstellungsanforderungen unterhalb der Mindestgröße.
** Die native Auswahl des GCNV Kapazitätspools wurde korrigiert, um ONTAP-Mode-Pools auszuschließen.
** Das Failover von KubeVirt für VMs, die Trident Automated Workload Failover verwenden, wurde behoben, um das VM-Failover zu beschleunigen.
** Behoben wurden RBAC-Probleme bei von Operatoren verwalteten Installationen, darunter fehlende Knoten ClusterRoles und fehlerhafte ClusterRole und ClusterRoleBinding Abgleiche während Upgrades. Dies betrifft Operator- und Helm-basierte Installationen, die das Trident Operator Chart verwenden.
** Persistente GitOps-Differenzen beim TridentConfigurator und den zugehörigen CRDs wurden durch das Weglassen der Standard-Prioritätswerte und die Korrektur der YAML-Vorlageneinrückung (link:https://github.com/NetApp/trident/issues/915["Problem #915"], link:https://github.com/NetApp/trident/issues/1120["Problem #1120"]) behoben.
** Behobenes Patchen von TridentVolumePublication CRDs während eines Helm-Upgrades (link:https://github.com/NetApp/trident/issues/1151["Problem #1151"]).
** Die Startreihenfolge von CSI wurde korrigiert, indem der CSI gRPC-Dienst vor den Wiederholungsversuchen der Knotenregistrierung gestartet wurde.
** Behobene sichere SMB-Klonkonfigurationsüberlagerungen bei gleichzeitigen Operationen.
** Fehlende Annotationen auf dem Trident Controller-Pod nach einem Upgrade behoben (link:https://github.com/NetApp/trident/issues/1004["Problem #1004"]).
** Verbesserte Bereitstellung langsamer Qtrees auf großen SVMs (link:https://github.com/NetApp/trident/issues/1142["Problem #1142"]).
** Behoben, dass ControllerPublish den Dateisystemtyp der Volume-Konfiguration anstelle eines falschen Standard-Dateisystemtyps verwendet.
** Ein Problem wurde behoben, bei dem verwaiste dm-Geräte wiederholt zu CSI-NodeUnstageVolume-Fehlern führen konnten.
** Verbesserte Bereinigung veralteter Knoten und Multipath-Systeme durch Entfernung von Ghost-Multipath-Artefakten und veralteten TridentNode-Ressourcen nach der Bereinigung von soft-gelöschten Knoten.
...
* Die Integration des OpenShift Service CA Operators wurde hinzugefügt, um die TLS-Zertifikatsverwaltung für den Trident Protect Admission Webhook zu automatisieren. Die Aktivierung erfolgt durch Setzen von  `useOpenshiftServiceCA` auf  `true` in den Helm-Werten. Weitere Informationen sind unter link:trident-protect/trident-protect-customize-installation.html["Trident Protect-Installation anpassen"^] verfügbar.
* Unterstützung für das Sichern und Wiederherstellen von KubeVirt-VMs auf OpenShift Virtualization hinzugefügt. Alle VMs in einem Namespace können geschützt oder gezielt bestimmte VMs anhand ihres Namens ausgewählt werden, zudem ist eine selektive Wiederherstellung einzelner VMs aus einem Backup oder Snapshot möglich. Weitere Informationen sind unter link:trident-protect/trident-protect-kubevirt-vms.html["Schutz von KubeVirt virtuellen Maschinen mit Trident Protect"^] zu finden.
* Es wurde Unterstützung für Ressourcentransformationen hinzugefügt, sodass Sie Kubernetes-Ressourcenattribute während des Wiederherstellungsprozesses hinzufügen, entfernen und ändern können. Weitere Informationen sind unter link:trident-protect/trident-protect-resource-transformations.html["Ändern Sie Ressourcen während der Wiederherstellung mithilfe benutzerdefinierter Ressourcen"] zu finden.
* Unterstützung für  `storageClassMapping` in AppMirrorRelationship (AMR) wurde hinzugefügt, wodurch die Zuordnung von Storage Classes für einzelne PVCs innerhalb einer einzigen AMR-Definition ermöglicht wird. Weitere Informationen sind unter link:trident-protect/trident-protect-use-snapmirror-replication.html["Anwendungen mit NetApp SnapMirror und Trident Protect replizieren"^] zu finden.
...
* Behoben wurde das Problem, dass die Wiederherstellung von virtuellen Maschinen zwischen Clustern mit einem Admission-Webhook-Fehler fehlschlug, wenn der Namespace der Quellanwendung auf dem Ziel-Cluster nicht vorhanden war.
* Selektive Wiederherstellungen vor Ort virtueller Maschinen, die beim Schritt der Anwendungserstellung aufgrund einer Spezifikationsabweichung mit einer vorhandenen benutzerdefinierten Anwendungsressource fehlschlugen, wurden behoben; die vorhandene Anwendung wird nun wiederverwendet.
* Behoben wurde, dass Wiederherstellung vor Ort von virtuellen Maschinen in einem  `Running` Zustand hängen blieb, wenn sowohl die virtuelle Maschine als auch ihr PersistentVolumeClaim gelöscht wurden; ein fehlender PersistentVolumeClaim wird nun als schwerwiegender Fehler behandelt.
* Die Bearbeitung eines bestehenden Schutzzeitplans löste fälschlicherweise eine sofortige Sicherung oder einen Snapshot-Lauf aus; dieses Verhalten wurde korrigiert.
* Es wurde ein Fehler behoben, bei dem der Snapshot-Finalizer versucht hat, die benutzerdefinierte Anwendungsressource nach deren Löschung abzurufen; der Finalizer überspringt diesen Vorgang nun, wenn die Anwendung nicht gefunden wird.
* Die Kapazität des festen geschützten Speichers erhöht sich nach einer Wiederherstellung vor Ort einer virtuellen Maschine durch die Verwendung von Sparse Copy während des PersistentVolumeClaim-Kopiervorgangs deutlich.
* Behoben: Trident Protect löschte das Kubernetes-Snapshot-Objekt, bevor die Löschung des zugrundeliegenden Snapshots in ONTAP bestätigt wurde; der Finalizer wartet nun auf  `VolumeSnapshot` Löschung.
* Behoben wurde, dass Snapshots nicht fortgesetzt wurden, wenn ein Einfrieren der virtuellen Maschine fehlschlug; Einfrieren und Auftauen werden nun auch bei einem Fehler fortgesetzt.
* Behoben wurde ein Fehler, bei dem KubeVirt virtuelle Maschinenanwendungen PersistentVolumeClaims während AppMirrorRelationship Reverse-, Reverse-Resync- und Resync-Operationen fälschlicherweise neu erstellt haben.
...
** Unterstützung für das automatische Volume-Wachstum in Trident basierend auf benutzerdefinierten Trident AutoGrow-Richtlinien wurde hinzugefügt. Siehe die link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["Dokumentation zur automatischen Volumenerweiterung"] für weitere Informationen.
** Verbesserte Trident-Knotenkonkurrenz zur Optimierung der Skalierbarkeit von Knotenoperationen für NVMe-Volumes. Siehe die link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["NVMe volumes Dokumentation"] für weitere Informationen.
...
** Unterstützung für die automatische Backend-Konfiguration für Amazon FSx for NetApp ONTAP hinzugefügt. Wenn Sie eine StorageClass erstellen, die die erforderlichen Parameter enthält, erstellt Trident automatisch das entsprechende Backend und die VolumeSnapshotClass, falls erforderlich. Siehe die link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAP Backend-Konfigurationsdokumentation"] für weitere Informationen.
** Es wurde Unterstützung für verschiedene Microsoft Azure-Clouds hinzugefügt, wie Azure Government und Azure China, sowie eine benutzerdefinierte Cloud-Konfiguration für Azure NetApp Files Backends. Siehe die link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Azure NetApp Files Backend-Konfigurationsdokumentation"] für weitere Informationen.
** Unterstützung für Kubernetes 1.35 hinzugefügt. Siehe die link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["Anforderungsdokumentation"] für weitere Informationen.
