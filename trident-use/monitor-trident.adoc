...
NOTE: Standardmäßig werden die Metriken von Trident auf dem Zielport angezeigt. `8001` am `/metrics` Endpunkt. Diese Metriken sind *standardmäßig aktiviert*, wenn Trident installiert ist. Sie können die Konfiguration so einstellen, dass Trident -Metriken über HTTPS auf Port 100 abgerufen werden. `8444` sowie.
...
Sie sollten ein Prometheus-Ziel definieren, um die Metriken zu erfassen und Informationen über die von Trident verwalteten Backends, die von ihm erstellten Volumes usw. zu erhalten. Sehenlink:https://prometheus-operator.dev/docs/developer/getting-started/["Prometheus Operator-Dokumentation"^] .
