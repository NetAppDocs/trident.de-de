...
| `snapshotDir` | Zugriff auf die `.snapshot` Verzeichnis | `true`, `false` (Das Standardverhalten kann abweichen. Stellen Sie es explizit auf „false“ für NFSv3 ein 
...
[%collapsible]
====
Bei der Installation von Trident mit einer GKE Cloud-Identität authentifizieren sich die Backends als Workload-Identität und lassen den  `apiKey` Block aus. Die Cloud-Identität wird für beide Google Cloud NetApp Volumes Treiber identisch konfiguriert. Der einzige Unterschied ist der  `storageDriverName` Wert:  `google-cloud-netapp-volumes` wird für NAS (NFS oder SMB) Volumes verwendet und  `google-cloud-netapp-volumes-san` für SAN (iSCSI Block) Volumes.


NOTE: Cloud-Identitätsunterstützung für den  `google-cloud-netapp-volumes-san` Treiber erfordert Trident 26.06 oder höher. SAN-Volumes werden aus Flex Unified Speicherpools bereitgestellt, daher  `serviceLevel` auf  `flex` setzen.

.NAS-Treiber (<code>google-cloud-netapp-volumes</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
----
.SAN Treiber (<code>google-cloud-netapp-volumes-san</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
