...
. Im primären Kubernetes-Cluster wird der TridentTridentSnapshotInfoCR erstellt, um interne Details zu erhalten.
+
.Beispiel
[source, yaml]
----
...
----
+
.. Im sekundären Kubernetes-Cluster das Feld _spec.state_ der _TridentMirrorRelationship_ CR auf _promoted_ und _spec.volumeMappings[].promotedSnapshotHandle_ auf den internalName des Snapshots aktualisieren.
