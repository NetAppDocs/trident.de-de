...
Weitere Informationen zu anderen Konfigurationsoptionen finden Sie im <<first-link,Erweiterte Backend Konfiguration und Beispiele>> Abschnitt unten.
...
Sie können SMB-Volumes mithilfe des  `ontap-nas` Treibers bereitstellen. Dazu müssen Sie jedoch die folgenden Schritte ausführen: link:worker-node-prep.html#prepare-to-provision-smb-volumes["Vorbereitung zur Bereitstellung von SMB Volumes"].



[[first-linkbackend-advanced-configuration-and-examples]]
== [[first-link]]Erweiterte Backend-Konfiguration und Beispiele
