...
'''
sidebar: sidebar permalink: trident-use/ontap-nas-examples.html Schlüsselwörter: Backends zuordnen, Storage Classes, Trident Backend, ontap-nas, ontap-nas-economy, ontap-nas-flexgroups, Optionen, Storageclasses, fsx, metrocluster Zusammenfassung: Es wird beschrieben, wie ONTAP NAS-Treiber mit einer Trident-Installation erstellt und verwendet werden. In diesem Abschnitt werden Beispiele für Backend-Konfigurationen und Details zur Zuordnung von Backends zu StorageClasses bereitgestellt. ---
