...
** Wenn die Rückgewinnungsrichtlinie der Storage Class `delete` ist, löscht Trident das Speichervolume, wenn das PV gelöscht wird.


* Standardmäßig verwaltet Trident die PVC und benennt das FlexVol Volume sowie die LUN oder den Qtree im Backend um. Es besteht die Möglichkeit, das  `--no-manage` Flag zu verwenden, um ein nicht verwaltetes Volume zu importieren, und das  `--no-rename` Flag, um den Volume-Namen beizubehalten.
+
...
**  `--no-rename`* - Wenn das  `--no-rename` Flag verwendet wird, behält Trident beim Importieren von Volumes den vorhandenen Volume-Namen bei und verwaltet den Lebenszyklus der Volumes. Diese Option wird nur für die  `ontap-nas`,  `ontap-san` (einschließlich ASA r2-Systeme),  `ontap-san-economy` und  `ontap-nas-economy` (qtree import) Treiber unterstützt.
+
...
[NOTE]
====
* Der  `ontap-nas-economy`Treiber verwaltet Speicher auf Qtree-Ebene, daher wird das eigenständige Importieren eines FlexVol Volumes nicht unterstützt. Zum Importieren von Qtrees mit dem  `ontap-nas-economy`Treiber siehe <<import-qtrees,Qtrees importieren>> (Tech Preview). Wenn Trident einen Qtree importiert, wird auch das übergeordnete FlexVol als Bucket importiert, das den Qtree enthält.
...
[NOTE]
====
Qtree Import ist eine Tech Preview Funktion in Trident 26.06. Tech Preview Funktionen sind für den Produktiveinsatz nicht unterstützt.

====
Ab Trident 26.06 ist es möglich, bestehende ONTAP qtrees mithilfe des  `ontap-nas-economy` Treibers in Trident zu importieren. Der Qtree wird durch seinen FlexVol Volume- und Qtree-Namen im  `flexvol/qtree` Format sowie durch die UUID des Backends, das ihn enthält, identifiziert. Die allgemeinen Importüberlegungen in <<Überblick und Überlegungen>> gelten auch für den Qtree-Import.

.Einen Qtree mit tridentctl importieren
Geben Sie den Qtree im  `flexvol/qtree` Format als Volume-Namen an. Im folgenden Beispiel wird der Qtree  `qtree1` aus dem FlexVol Volume  `flexvol1` auf dem Backend  `ontap_nas_economy` importiert:

[listing]
----
tridentctl import volume ontap_nas_economy flexvol1/qtree1 -f <path-to-pvc-file>
----
Um den Qtree zu importieren, ohne ihn umzubenennen, das  `--no-rename` Flag hinzufügen. Um den Qtree zu importieren, ohne dass Trident seinen Lebenszyklus verwaltet, das  `--no-manage` Flag hinzufügen.

.Import eines Qtrees mithilfe von PVC-Annotationen
Eine PVC mit den folgenden Annotationen erstellen:
...
spec:
  ...
  storageClassName: <storage-class-name>
----
Um die bestehenden FlexVol volume- und qtree-Namen beizubehalten, die  `trident.netapp.io/importNoRename: "true"` Annotation hinzufügen. Um den qtree als nicht verwaltet zu importieren, die  `trident.netapp.io/notManaged: "true"` Annotation hinzufügen.

.Benennungs- und Managementverhalten
Verwalteter Import (Standard):: Trident benennt den Qtree entsprechend dem PVC-Namen um und benennt das FlexVol volume gemäß der  `ontap-nas-economy` Namenskonvention um. Der Verbindungspfad bleibt unverändert.
`--no-rename`:: Trident behält die bestehenden FlexVol volume- und qtree-Namen bei. Die Namen müssen bereits den Trident Namenskonventionen entsprechen, sonst kann Trident keine Lösch- oder Mount-Operationen für den qtree verwalten.
`--no-manage`:: Trident referenziert den qtree, verwaltet aber nicht dessen Lebenszyklus. Die Namen müssen den Trident Namenskonventionen entsprechen, und Trident verwendet oder verwaltet keine Auto-Export-Richtlinien für den qtree.


.Verhalten der Export-Policy
Wenn das Backend automatische Exportrichtlinien verwendet, nutzt der importierte Qtree das Exportrichtlinienmodell pro Qtree, bei dem jeder Qtree seine eigene Exportrichtlinie besitzt. Trident weist dem Qtree beim Import eine leere Exportrichtlinie zu. Beim Veröffentlichen aktualisiert Trident das FlexVol volume auf die Exportrichtlinie pro Backend, falls der automatische Export aktiviert ist und die bestehende FlexVol Exportrichtlinie  `default` oder leer ist.

[CAUTION]
====
Die Aktualisierung der Export-Richtlinie des FlexVol Volumes kann andere Qtrees im selben FlexVol Volume beeinträchtigen, die derzeit eingebunden sind. Qtrees sollten nur importiert werden, wenn die Qtrees im FlexVol Volume ausgehängt sind.

====
[WARNING]
====
Zwei Kubernetes-Cluster dürfen nicht auf dieselbe aktive SVM verweisen, und es darf kein Qtree importiert werden, der noch von einem anderen Cluster verwaltet wird. Wenn beide Cluster dasselbe Volume verwalten, können widersprüchliche Vorgänge die Daten beschädigen oder zerstören.
