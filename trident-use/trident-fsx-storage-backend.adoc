...
Die Schritte in diesem Abschnitt beschreiben, wie Sie das Backend separat erstellen. Informationen zur Verwendung der Funktion „Automatisches Backend“ finden Sie im link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["Konfigurieren StorageClass und PVC"] Abschnitt.
...
| `storageDriverName` | Name des Speichertreibers. Kann nach der Erstellung nicht mehr geändert werden. Zum Aktualisieren dieses Parameters ist die Erstellung eines neuen Backends erforderlich. | `ontap-nas`, `ontap-nas-economy`, `ontap-nas-flexgroup`, `ontap-san`, `ontap-san-economy` 
...
| `storagePrefix` | Das Präfix wird beim Bereitstellen neuer Volumes in der SVM verwendet. Kann nach der Erstellung nicht geändert werden. Um diesen Parameter zu aktualisieren, müssen Sie ein neues Backend erstellen. | `trident`HINWEIS: Backend-Konfigurationsparameter, die die Backend-Identität und die Benennung von Volumes beeinflussen, wie  `storageDriverName` und  `storagePrefix`, können nach der Erstellung nicht mehr geändert werden. Um diese zu aktualisieren, ist die Erstellung eines neuen Backends erforderlich. 
...
Sie können SMB-Volumes mithilfe des  `ontap-nas` Treibers bereitstellen. Bevor Sie link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["ONTAP SAN- und NAS-Treiberintegration"] abschließen, führen Sie diese Schritte aus: link:worker-node-prep.html#prepare-to-provision-smb-volumes["Vorbereitung zur Bereitstellung von SMB Volumes"].
