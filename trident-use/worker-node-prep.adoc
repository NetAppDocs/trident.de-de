...
.SMB -Tools
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Vorbereitung zur Bereitstellung von SMB Volumes"] wenn Sie Folgendes verwenden: `ontap-nas` zur Bereitstellung von SMB-Volumes.
...
Sie können SMB-Volumes bereitstellen mit `ontap-nas` Fahrer.
