...
Durch Verwendung der FC-Funktion mit Trident sind folgende Möglichkeiten möglich: * VES dynamisch mithilfe einer Implementierungsspezifikation bereitstellen. * Nehmen Sie Volume-Schnappschüsse und erstellen Sie ein neues Volumen aus dem Snapshot. * Klonen Sie eine vorhandene FC-PVC. * Ändern Sie die Größe eines bereits bereitgestellten Volumes.
