...
Wenn Sie AppVault-Objekte mit CRS oder dem Trident Protect CLI-Plugin erstellen, können Sie optional Trident Protect anweisen, einen Kubernetes-Schlüssel zu verwenden, der benutzerdefinierte Passwörter für die Restic- und Kopia-Repository-Verschlüsselung enthält. Wenn Sie keinen Schlüssel angeben, verwendet Trident Protect `password` für beide Passwörter.

* Wenn Sie AppVault CRS manuell erstellen, können Sie das Feld *spec.properties.dataMoverPasswordSecretRef* verwenden, um den Schlüssel anzugeben.
...
----
--
.Verwenden Sie die Trident Protect-CLI
--
[source, console]
----
