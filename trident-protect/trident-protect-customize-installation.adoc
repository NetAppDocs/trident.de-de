...
Sie können die AutoSupport-Einstellungen, die Namensraumfilterung und die OpenShift-Webhook-Zertifikatverwaltung an Ihre spezifischen Anforderungen anpassen. Die folgende Tabelle beschreibt die verfügbaren Konfigurationsparameter:
...
| useOpenshiftServiceCA | boolesch | Ermöglicht die Integration mit OpenShifts nativem Service CA Operator für die automatische Erstellung und Einbindung von TLS-Zertifikaten für den Trident Protect Admission-Webhook. Wenn auf  `true` gesetzt, verwaltet der Service CA Operator die Webhook-TLS-Zertifikate automatisch. Nur auf OpenShift-Clustern verwenden. (Standard:  `false`) 
|===
