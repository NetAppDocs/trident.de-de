...
NOTE: Um eine VM-bezogene Anwendung zu definieren, die bestimmte KubeVirt virtuelle Maschinen (VMs) anhand ihres Namens anstatt ganzer Namensräume schützt, kann das  `spec.includedVirtualMachines` Feld im Application-CR oder das  `--virtual-machines` Flag in der CLI verwendet werden. Details, Feldreferenz und YAML-Beispiele sind unter link:trident-protect-kubevirt-vms.html["Schutz von KubeVirt virtuellen Maschinen mit Trident Protect"] zu finden.
...
+
Um eine reine PVC-Anwendung zu definieren, müssen Sie auch  `PersistentVolume` und  `VolumeSnapshotClass` im Ressourcenfilter angeben. Snapshot- und Backup-Vorgänge hängen von  `PersistentVolume` (dem clusterweiten Volume, das an jedes PVC gebunden ist) und  `VolumeSnapshotClass` (dem Snapshot-Treiber) ab und schlagen ohne sie fehl. Beispiel:
...
+ Um die Annotation bei der Erstellung einer Anwendung über die CLI anzugeben, können Sie das  `--annotation` Flag verwenden.

+ * Die Anwendung wird erstellt und die globale Einstellung für das Einfrieren des Dateisystems verwendet:
...
----
+ * Die Anwendung wird erstellt und die lokale Anwendungseinstellung für das Verhalten beim Einfrieren des Dateisystems wird konfiguriert:
...
----
+ * Sie können  `--resource-filter-include` und  `--resource-filter-exclude` Flags verwenden, um Ressourcen basierend auf  `resourceSelectionCriteria` wie Gruppe, Art, Version, Bezeichnungen, Namen und Namensräumen einzuschließen oder auszuschließen, wie im folgenden Beispiel gezeigt:
...
----
Um eine reine PVC-Anwendung zu definieren, müssen auch  `PersistentVolume` und  `VolumeSnapshotClass`` im Ressourcenfilter enthalten sein. Snapshot- und Backup-Vorgänge hängen von  `PersistentVolume` (dem clusterweiten Volume, das an jedes PVC gebunden ist) und  `VolumeSnapshotClass` (dem Snapshot-Treiber) ab und schlagen ohne diese fehl. Beispiel:
