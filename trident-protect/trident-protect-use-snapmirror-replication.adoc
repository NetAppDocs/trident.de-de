...
NOTE: Bei Verwendung eines CR muss der Ziel-Namespace vor dem Anwenden des CR manuell erstellt werden. Trident Protect erstellt Namensräume automatisch nur bei Verwendung der Befehlszeilenschnittstelle (CLI).
...
*** *spec.storageClassMapping*: (_Optional_) Ordnet Quellspeicherklassen Zielspeicherklassen für einzelne PVCs innerhalb desselben AMR zu. Jeder Eintrag erfordert einen  `source` und  `destination` Namen für die Speicherklasse. Wenn festgelegt,  `storageClassMapping` überschreibt  `storageClassName` vollständig; PVCs ohne übereinstimmenden Quelleintrag verwenden die Standardspeicherklasse des Clusters, nicht  `storageClassName`.
+
Bei der Verwendung von  `storageClassMapping` in einem AppMirrorRelationship CR sind die folgenden Einschränkungen zu beachten:

+
**** `storageClassMapping`überschreibt `storageClassName`vollständig. Wenn beide Felder gesetzt sind,  `storageClassName`wird ignoriert.
**** PVCs mit Quellspeicherklassen, die keinen passenden Eintrag in `storageClassMapping` haben, verwenden die Standardspeicherklasse des Clusters, nicht `storageClassName`.
**** Jeder Eintrag erfordert sowohl ein  `source` als auch  `destination` Wert.
**** Durch die Änderung der Zielspeicherklasse kann das Löschen und Neuerstellen vorhandener Ziel-PVCs erzwungen werden.
...
NOTE: Verwenden Sie  `--storageclass-mapping` zur Zuordnung einzelner Quell-Speicherklassen zu Ziel-Speicherklassen. Ein oder mehrere  `source:destination`-Paare, durch Kommas getrennt (zum Beispiel  `"class-a:class-b,class-c:class-d"`), können angegeben werden. Verwenden Sie  `*:destination-class` als Platzhalter, um alle nicht zugeordneten PVCs einer einzigen Ziel-Speicherklasse zuzuordnen. Wenn  `--storageclass-mapping` festgelegt ist, wird  `--storage-class` ignoriert.

+
*Beispiel mit einer einzelnen Zielspeicherklasse (`--storage-class`):*
