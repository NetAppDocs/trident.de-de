...
. Führen Sie die Schritte für die umgekehrte Resynchronisierung in <<Reverse resync a failed over replication>>aus.
...
* Die Beziehung rückgängig machen.
* Die Replikation wird rückgängig gemacht.


.Schritte
. Führen Sie die <<Resync a failed over replication>> Schritte aus
. Führen Sie die <<Reverse resync a failed over replication>> Schritte aus
