...
* Sie können erweiterte Einstellungen verwenden, um Wiederherstellungsvorgänge anzupassen. Weitere Informationen finden Sie unter link:../trident-protect/trident-protect-restore-settings.html["Verwenden Sie die erweiterten Trident Protect-Wiederherstellungseinstellungen."].
* Um bestimmte KubeVirt virtuelle Maschinen (VMs) aus einem Archiv wiederherzustellen, ohne alle Namespace-Ressourcen wiederherzustellen, kann das  `includedVirtualMachines` Feld in einem Restore-CR verwendet werden. Unterstützte CRs, Feldreferenzen und YAML-Beispiele sind unter link:trident-protect-kubevirt-vms.html["Schutz von KubeVirt virtuellen Maschinen mit Trident Protect"] zu finden.
...
* Wenn Sie eine CR verwenden, um in einem neuen Namespace wiederherzustellen, müssen Sie den Ziel-Namespace manuell erstellen, bevor Sie die CR anwenden. Trident Protect erstellt Namensräume automatisch nur bei Verwendung der Befehlszeilenschnittstelle (CLI).
...
[NOTE]
====
* SnapshotRestore unterstützt die `spec.storageClassMapping` Attribut, aber nur, wenn die Quell- und Zielspeicherklassen dasselbe Speicher-Backend verwenden.  Wenn Sie versuchen, eine `StorageClass` das ein anderes Speicher-Backend verwendet, schlägt der Wiederherstellungsvorgang fehl.
* Wenn Sie eine CR verwenden, um in einem neuen Namespace wiederherzustellen, müssen Sie den Ziel-Namespace manuell erstellen, bevor Sie die CR anwenden. Trident Protect erstellt Namensräume automatisch nur bei Verwendung der Befehlszeilenschnittstelle (CLI).
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ . (_Optional_) Falls nur bestimmte Ressourcen der Anwendung für die Wiederherstellung ausgewählt werden sollen, kann ein Filter hinzugefügt werden, der Ressourcen mit bestimmten Bezeichnungen ein- oder ausschließt:

+ HINWEIS: Trident Protect wählt einige Ressourcen automatisch aus, weil sie mit den von Ihnen ausgewählten Ressourcen in Beziehung stehen. Wenn beispielsweise eine Ressource für einen Persistent Volume Claim ausgewählt wird und diese einen zugehörigen Pod hat, stellt Trident Protect auch den zugehörigen Pod wieder her.

+ * *resourceFilter.resourceSelectionCriteria*: (Für die Filterung erforderlich)  `Include` oder  `Exclude` verwenden, um eine in resourceMatchers definierte Ressource ein- oder auszuschließen. Die folgenden resourceMatchers-Parameter hinzufügen, um die einzuschließenden oder auszuschließenden Ressourcen zu definieren: ** *resourceFilter.resourceMatchers*: Ein Array von resourceMatcher-Objekten. Wenn mehrere Elemente in diesem Array definiert werden, erfolgt die Übereinstimmung als ODER-Verknüpfung, und die Felder innerhalb jedes Elements (group, kind, version) werden als UND-Verknüpfung betrachtet. *** *resourceMatchers[].group*: (_Optional_) Gruppe der zu filternden Ressource. *** *resourceMatchers[].kind*: (_Optional_) Art der zu filternden Ressource. *** *resourceMatchers[].version*: (_Optional_) Version der zu filternden Ressource. *** *resourceMatchers[].names*: (_Optional_) Namen im Kubernetes-Feld metadata.name der zu filternden Ressource. *** *resourceMatchers[].namespaces*: (_Optional_) Namespaces im Kubernetes-Feld metadata.name der zu filternden Ressource. *** *resourceMatchers[].labelSelectors*: (_Optional_) Label-Selektorzeichenfolge im Kubernetes-Feld metadata.name der Ressource, wie in der  https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Kubernetes-Dokumentation"^] definiert. Zum Beispiel:  `"trident.netapp.io/os=linux"`.
  ...
  resourceFilter:
    ...
    resourceMatchers:
      ...
      - group: my-resource-group-2
        ...
        labelSelectors: ["trident.netapp.io/os=linux"]
----
+ . Nachdem die  `trident-protect-snapshot-ipr-cr.yaml` Datei mit den korrekten Werten gefüllt wurde, wird die CR angewendet:
