...
In `/deploy/operator.yaml` , aktualisieren `image: docker.io/netapp/trident-operator:25.10.0` um den Speicherort Ihrer Bildregistrierung widerzuspiegeln. Deinlink:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident und CSI-Images"] Die CSI-Images können sich in einem oder mehreren Registrys befinden, aber alle CSI-Images müssen sich im selben Registry befinden. Beispiel:

* `image: <your-registry>/trident-operator:25.10.0`wenn sich alle Ihre Bilder in einem einzigen Verzeichnis befinden.
* `image: <your-registry>/netapp/trident-operator:25.10.0`wenn sich Ihr Trident Image in einem anderen Verzeichnis befindet als Ihre CSI-Images.
