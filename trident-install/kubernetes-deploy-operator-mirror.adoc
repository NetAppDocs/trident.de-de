...
In `/deploy/operator.yaml`  `image: docker.io/netapp/trident-operator:26.06.0`aktualisieren Sie den Pfad, um den Speicherort Ihrer Image-Registry widerzuspiegeln. Ihre link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident und CSI-Images"] können sich in einer Registry oder in verschiedenen Registrys befinden, aber alle CSI-Images müssen sich in derselben Registry befinden. Beispiel:

* `image: <your-registry>/trident-operator:26.06.0` wenn sich alle Ihre Images in einer einzigen Registry befinden.
* `image: <your-registry>/netapp/trident-operator:26.06.0`wenn sich Ihr Trident-Image in einem anderen Registry als Ihre CSI-Images befindet.
