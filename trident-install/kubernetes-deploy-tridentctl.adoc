...
[%collapsible]
====
Um gleichzeitige Trident Controller-Operationen für verbesserten Durchsatz zu ermöglichen, kann die  `--enable-concurrency` Option während der Installation wie in diesem Beispiel hinzugefügt werden. Weitere Informationen zu unterstützten Treibern sind unter folgendem Link zu finden: https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation].
