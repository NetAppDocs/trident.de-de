...
[]
=====
* Kubernetes wird jetzt in Trident unterstützt. Trident sollte vor Kubernetes aktualisiert werden.
...
Ob manuell oder mit Helm – der Trident-Operator ist eine großartige Möglichkeit, die Installation zu vereinfachen und Trident-Ressourcen dynamisch zu verwalten. Sie können sogar link:kubernetes-customize-deploy.html["Individuelle Anpassung der Trident Implementierung"] mithilfe der Attribute in der  `TridentOrchestrator` benutzerdefinierten Ressource (CR) vorgehen.
